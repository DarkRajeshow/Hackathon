...
import Udemy_Certificate from '@/public/Udemy_Certificate.jpeg'
...
export default function Page() {
  ...
  const handleSubmit = () => {
    ...
    setTimeout(() => {
      if (hashCode !== "abcd") {
        setTextMessage("Invalid Hash code.");
  ...
  return (
    ...
    <>
      <ScreenLoading loadState={loadState} showDocument={showDocument} textMessage={textMessage} checkText={"File Retrived Successfully"} />
      <article className='p-4 py-10 sm:p-6 md:p-8 lg:p-10'>
        ...
        <div className='flex flex-col items-center py-10 bg-dark rounded-lg my-10 text-light'>
          ...
          <div className='flex gap-1 sm:gap-2 md:gap-4 w-[90%] justify-center'>
            <input value={hashCode} onChange={(event) => { setHashCode(event.target.value) }} type="text" name="" id="" placeholder='Enter your hash code here...' className='p-1 px-2  md:py-2 md:px-3 rounded-md text-lg w-[60%] sm:w-[60%] md:w-[40%] lg:w-[30%] text-dark font-semibold' />
        ...
        <div className='flex flex-col items-start px-6 sm:px-8 md:px-10 lg:px-20 py-4 sm:py-6 md:py-8 lg:py-10 bg-dark rounded-lg my-4 sm:my-6 md:my-8 lg:my-10 text-light'>
          <h3 className='text-2xl md:text-3xl lg:text-4xl font-bold py-4'>Your file!</h3>
          {showDocument && <Image src={Udemy_Certificate} alt='Certificate' />}
          {!showDocument && <div className='text-center w-full py-20 bg-light/10 rounded-md ' >
